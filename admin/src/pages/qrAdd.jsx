 ...
 *  @version: 1.0.0
 *  @author: Sergio
 *  @description: Página de agregar QR: /QR/nuevo
*/
...
const PageQRAdd = (props) => {
    ...
    /**
     * @name: handleChange
     * @description: Función para designar el tipo de QR : "url", "text"
    */
    ...
    */
    const handleSubmit = async (event) => {
        ...
        const { result, error } = await request.post('/qrs/add', data);
        if (result && !result.error) {
            if (result.created) {
                cogoToast.success("QR añadido exitosamente");
                setTimeout(() => {
    ...
    return(
        <Layout title = "Nuevo QR">
            <CssBaseline />
            <Paper className = { classes.root }>
                <form className = { classes.form } noValidate onSubmit = { handleSubmit }>
                <FormControl component="fieldset" >
                    <FormLabel component="legend">Tipo de QR</FormLabel>
                    ...
                    {   value === "url" ?
                            <TextField className = { classes.inputs } name="url_target" required label="URL de envío al QR" /> :
                        value === "text" ?
                            <TextField className = { classes.inputs } name="target_message" required label="Mensaje del QR" /> : null
                    }
...
export default withRouter(PageQRAdd);